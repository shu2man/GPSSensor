...
public class MYSQL{
    ...
    public void createtable(){
            db.execSQL("create table if not exists drawLatLngs(_id integer primary key autoincrement," +
                    "users_id varchar(10),drawLatLngs_id varchar(20))");
    ...
    public void new_user(String name,String password,String icon)
    {
        try {
            db.execSQL("insert into users values(?,?,?,?,?)",
                    new String[]{Integer.toString(user_getCount()),name,password,icon,Integer.toString(0)});
        }
    ...
    public void update_user(String users_id,String name,String password,String icon)
    {
        try {
            db.execSQL("update users set name = ? where users_id = ?",new String[]{name,users_id});
            db.execSQL("update users set password = ? where users_id = ?",new String[]{password,users_id});
            db.execSQL("update users set icon = ? where users_id = ?",new String[]{icon,users_id});
        }
        catch (SQLiteException e) {}
    }
    //    drawLatLngs
    public void save_drawLatLngs(String users_id,ArrayList<LatLng> drawLatLngs)
    {
        ...
        try {
            db.execSQL("update users set drawLatLngs_num = drawLatLngs_num+1 where users_id = ?",new String[]{users_id});
            db.execSQL("insert into drawLatLngs values(?,?,?)", new String[]{null, users_id, drawLatLngs_id});
            ...
            for (int i = 0; i < drawLatLngs.size(); i++) {
                db.execSQL("insert into " + users_id + " values(?,?,?)",
                        new String[]{null, Double.toString(drawLatLngs.get(i).x), Double.toString(drawLatLngs.get(i).y)});
            }
        }
        catch (SQLiteException e) {}
    }
